...
exports.isInterestingBP = exports.isInteresting = void 0;
/**
 ・面白い数かどうか
    -
...
function isInterestingNumber(n, awesomePhrases) {
    if (n <= 99)
        return false;
    if (isAllZeroFollowed(n))
        return true;
    if (isEveryDigitSame(n))
        return true;
    if (isSequential(n))
        return true;
    ...
    if (isPalindrome(n))
        return true;
    if (isIncludeAwesome(n, awesomePhrases))
        return true;
    return false;
}
function isAllZeroFollowed(n) {
    return n.toString().split('').slice(1).every(function (v) { return v === "0"; });
}
function isEveryDigitSame(n) {
    var s = n.toString()[0];
    return n.toString().split('').every(function (v) { return v === s; });
}
function isSequential(n) {
    var arrInt = n.toString().split('').map(function (v) { return parseInt(v); });
...
function isPalindrome(n) {
    var strN = n.toString();
    for (var i = 0; i < strN.length / 2; i++) {
        if (strN[i] !== strN[strN.length - 1 - i])
            return false;
    }
    return true;
}
function isIncludeAwesome(n, awesomePhrases) {
    return awesomePhrases.includes(n);
}
function isNearInterestingNumber(n, awesomePhrases, near) {
    var plus = 1;
    while (plus <= near) {
 ...
 * BP
 */
function isInterestingBP(n, awesomePhrases, iteration, awesomeness) {
    if (iteration === void 0) { iteration = 0; }
    if (awesomeness === void 0) { awesomeness = 2; }
    switch (true) {
...
exports.isInterestingBP = isInterestingBP;
//# sourceMappingURL=4 kyu Catching Car Mileage Numbers.js.map